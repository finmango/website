...
| `/post-image?id=…[&f=…][&w=480\|800\|1200\|1600]` | `functions/post-image.js` | Serves the post's cover from our own domain, **edge-cached** (Google Drive thumbnail URLs are flaky for crawlers and slow for visitors). `f` is the cover's Drive file id — the pages pass it along so the proxy fetches the file directly instead of re-asking Apps Script which file the cover is (a lookup that fails during backend cold starts and used to leave the article hero showing the default card while `/posts` showed the real cover). Without `f` the post is looked up by `id`. The optional `w` asks Drive for a right-sized thumbnail, so grid cards load an ~800px image instead of the raw 2000px original. Used by the `og:image`, the `posts.html` cards, and the article cover on `post.html` (which `/post` also preloads for a fast LCP). Falls back to `og-image.png` (uncached) when a post has no cover or the image can't be fetched. |
